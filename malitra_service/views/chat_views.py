...
from rag_agent import process_question
...
class ChatView(APIView):
    ...
    def post(self, request):
        ...
        # Jalankan proses RAG Agent (Langchain + Postgres)
        try:
            result = process_question.invoke({"question": question})
            final_answer = result.get("final_answer", "Maaf, terjadi kesalahan saat memproses pertanyaan.")
        except Exception as e:
            return Response({"status": 500, "error": str(e)}, status=500)

        # Simpan ke Database Chat History
        try:
            ...
            return Response({"status": 500, "error": f"Gagal menyimpan chat: {str(e)}"}, status=500)

        # Response ke Frontend
        response_data = {"user_id": user.id, "answer": final_answer}
